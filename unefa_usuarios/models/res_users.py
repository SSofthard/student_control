...
def buscar_cedula_saime_general(cedula):
        val = {}
        mensaje = {}
        if cedula:
            db_o = 'saime'
            ...
            conn0 = psycopg2.connect(
                        dbname=db_o, user=username_o, password=password_o,
                        host='localhost'
                    )
            cur0 = conn0.cursor()
            cur0.execute(
                "SELECT primer_nombre, segundo_nombre, primer_apellido, \
                    segundo_apellido, fecha_nacimiento, origen, sexo\
                FROM saime \
                WHERE cedula=%s",(int(cedula),)
            )
            rows = cur0.fetchall()
            cur0.close()
            conn0.close()
            ...
            if len(rows) == 0:
                mensaje={
                        ...
                        'message':('No se encontraron resultados en la base datos\
                                    del SAIME...'),
                }
                val={
                    'name': '',
...
class unefa_usuarios(osv.osv):
    ...
    _columns = {
        ...
        'nacionalidad':fields.selection([('extrangero','Extranjero'),('venezolano','Venezolano')],'Nacionalidad', required=True),
        'cedula':fields.char('Cédula', required=True, size=8, help = 'Ingrese el número de Cédula sin puntos.'),
        'telefono_local':fields.char('Teléfono Local', required=False),
        'telefono':fields.char('Teléfono Celular', required=False),
        'fecha_nacimiento':fields.date('Fecha de Nacimiento', required=False),
        'estado_civil':fields.selection([('soltero','Soltero'),('casado','Casado'),('divorciado','Divorciado'),('viudo','Viudo')],'Estado Civil', required=False),
        'sexo':fields.selection([('masculino','Masculino'),('femenino','Femenino')],'Sexo', required=False),
        'condicion':fields.selection([('civil','Civil'),('militar','Militar')],'Condición', required=False),
        ...
        'estado_id':fields.many2one('unefa.estados','Estado', required=False),
        'municipio_id':fields.many2one('unefa.municipios','Municipio', required=False),
        'parroquia_id':fields.many2one('unefa.parroquias','Parroquia', required=False),
        'sector':fields.char('Sector', required=False, size=120),
        'calle_avenida':fields.char('Calle/Avenida', required=False, size=80),
        'casa_apto':fields.char('Casa/Apto', required=False, size=80),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals['sector'] != False:
            vals.update({
                'sector':vals['sector'].upper(),
                })
        if vals['calle_avenida'] != False:
            vals.update({
                'calle_avenida':vals['calle_avenida'].upper(),
                })
        if vals['casa_apto'] != False:
            vals.update({
...
class unefa_usuarios_coordinador(osv.osv):
    ...
    def buscar_cedula_saime(self, cr, uid, ids, cedula, context=None):
        return buscar_cedula_saime_general(cedula)
...
class unefa_usuarios_asistente(osv.osv):
    ...
    def create(self,cr,uid,vals,context=None):
        filtro_carrera=self.coordinacion_default(cr,uid,[])
...
class unefa_usuarios_estudiante(osv.osv):
    ...
    _columns = {
        ...
        'semestre_id':fields.many2one('unefa.semestre','Semestre', required=True, help = "Semestre actual  del Estudiante."),
        }
    ...
    def create(self,cr,uid,vals,context=None):
        ...
        buscar_cedula = self.buscar_cedula_saime(cr,uid,[],vals['cedula'])
        vals.update({
                    'name': buscar_cedula['value']['name'],
                    'segundo_nombre': buscar_cedula['value']['segundo_nombre'],
                    'primer_apellido':buscar_cedula['value']['primer_apellido'],
                    'segundo_apellido': buscar_cedula['value']['segundo_apellido'],
                    'fecha_nacimiento': buscar_cedula['value']['fecha_nacimiento'],
                    'nacionalidad': buscar_cedula['value']['nacionalidad'],
                    'sexo': buscar_cedula['value']['sexo'],
                    })
...
class unefa_dedicacion_profesor(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        return super(uidnefa_dedicacion_profesor, self).write(cr, uid, ids, vals, context=context)
