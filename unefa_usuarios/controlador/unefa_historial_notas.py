...
class listas_generales_estudiantes(http.Controller):
    ...
    def descargar_lista_general_estudiante(self,estudiante_data, **post):
        ...
        nota_migrada_obj=registry('unefa.notas_migracion')
        nota_migrada_id = nota_migrada_obj.search(cr,uid,[('estudiante_id','=',estudiante_id)])
        nota_migrada_data = nota_migrada_obj.browse(cr,uid,nota_migrada_id)
        print 'jeison'
        print 'jeison'
        print nota_migrada_id
        if len(nota_migrada_id):
            for notam in nota_migrada_data:
                vals['asignaturam']=notam.asignatura_id.asignatura
                vals['calificacionm']=notam.calificacion
                list_nota_asignatura.append(vals)
        print 'list_nota_asignatura'
        print list_nota_asignatura
        ...
        for inscripcion in inscripcion_data:
            for asignatura in inscripcion.asignaturas_inscritas_ids:
                ...
                gestion_id=gestion_obj.search(cr,uid,[('asignatura_id','=',asignatura.asignatura_id.id),
                                                    ('periodo_id','=',inscripcion.periodo_id.id),
