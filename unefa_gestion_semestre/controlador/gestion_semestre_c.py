...
class unefa_gestionar_semestre(http.Controller):
    ...
    @http.route(['/descargar/listas_estudiantes_semestre/<model("unefa.generar_lista_semestre"):lista_semestre_data>'],
                type='http', auth='user', website=True)
    def descargar_lista_estudiantes_semestre(self,lista_semestre_data, **post):
        cr, uid, context = request.cr, request.uid, request.context
        registry = http.request.registry
        list_estudiante_id=[]
        list_semestre_ids=[]
    ...
    @http.route(['/descargar/acta_notas_recuperacion/<int:id_gestion>'],
                type='http', auth='user', website=True)
    def descargar_acta_notas_recuperacion(self,id_gestion, **post):
        cr, uid, context = request.cr, request.uid, request.context
        registry = http.request.registry
        
        gestion_obj=registry('unefa.gestion_semestre')
        gestion_id=gestion_obj.search(cr,uid,[('id','=',int(id_gestion))])
